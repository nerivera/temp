...
USER_POOL_ID = 'us-west-2_ds8sw22B5'
            ...
            if item['type'] == server_type:
                ...
                if not emails_sent(username=item["username"], instance_start=item["snapshot"].started, inactivity_start=item["snapshot"].last_activity, inactive_hours=item["inactive_hours"]):
                    if item["inactive_hours"] in idle_email_hour_user:
                        ...
                        send_idle_email(item["username"],'jeffrey.k.pon@jpl.nasa.gov', item['type'], item["last_activity"], item["inactive_hours"], item["started"], item['started_hours'])
                    elif item["inactive_hours"] == idle_email_hour_ondemand_slack:
                        send_idle_email_ondemand_help(item["username"],'nisar-sds-ondemand-aaaadidrtulqmfpmqkpooiqlby@jpl.org.slack.com', item['type'], item["last_activity"], item['inactive_hours'], item["started"], item['started_hours'])
